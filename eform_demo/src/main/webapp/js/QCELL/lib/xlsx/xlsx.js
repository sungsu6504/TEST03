...
var HTML_ = (function() {
	...
	function make_html_row(ws, r, R, o) {
		...
		for(var C = r.s.c; C <= r.e.c; ++C) {
			...
			sp.id = "rt-qc-" + coord;
			sp.class = "rt-qc-cell rt-qc-td";
			sp["data-row"] = R;
			sp["data-col"] = C;
			//oo.push(writextag('td', ''+w, sp));
			oo.push(writextag('td', ''+cell.v, sp));
		}
		var preamble = "<tr class='rt-qc-cell rt-qc-tr'>";
		return preamble + oo.join("") + "</tr>";
	}
	function make_html_preamble(ws, R, o) {
		var out = [];
		return out.join("") + '<table id="rt-qc-tbl" class="rt-qc-root rt-qc-table">';
	}
	var _BEGIN = '<html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /><meta charset="utf-8"/><title>HTML Export</title></head><body>';
	var _END = '</body></html>';
...
function sheet_to_json_range(sheet, opts) {
	if(sheet == null || sheet["!ref"] == null) return [];
	var val = {t:'n',v:0}, header = 0, offset = 1, hdr = [], isempty = true, v=0, vv="";
	var r = {s:{r:0,c:0},e:{r:0,c:0}};
	var o = opts || {};
	...
	switch(typeof range) {
		case 'string': r = safe_decode_range_from_zero(range); break;
		case 'number': r = safe_decode_range_from_zero(sheet["!ref"]); r.s.r = range; break;
		default: r = range;
	}
	return r.e.c;
}

function sheet_to_json_save_empty(sheet, opts) {
	if(sheet == null || sheet["!ref"] == null) return [];
	var val = {t:'n',v:0}, header = 0, offset = 1, hdr = [], isempty = true, v=0, vv="";
	var r = {s:{r:0,c:0},e:{r:0,c:0}};
	var o = opts || {};
...
function safe_decode_range_from_zero(range) {
	var o = {s:{c:0,r:0},e:{c:0,r:0}};
	var idx = 0, i = 0, cc = 0;
	var len = range.length;
	for(idx = 0; i < len; ++i) {
...
var utils = {
	...
	sheet_to_json_save_empty: sheet_to_json_save_empty,
	sheet_to_json_range: sheet_to_json_range
};
